...
Empirical evaluation is conducted on the official research dataset of Zhang et al. (2017), using hourly records from the Aotizhongxin monitoring station ($N = 35{,}064$ contiguous hours, 2013–2017). A controlled ablation study (Models A–D) quantifies the contribution of the DAE and attention blocks under both clean and corrupted test conditions. On the clean test partition, the full model (Model A) achieves $R^2 = 0.9159$; under Gaussian corruption ($\sigma = 0.12$) applied to scaled auxiliary sensor channels, Model A attains $R^2 = 0.9170$, outperforming the no-DAE variant (Model B: $R^2 = 0.8692$) by $\Delta R^2 = +0.0477$. Attention visualisation reveals elevated temporal weights during morning and evening rush-hour bands, consistent with traffic-related emission dynamics. These results demonstrate that denoising representation learning and temporal attention jointly preserve high forecast accuracy under both idealised and stress-tested sensor conditions.
...
Publication tables are maintained in `outputs/ablation_tables.md` (generated by `visualization.py`; regenerate via `python generate_visualizations.py` after `python main.py`). Metrics below correspond to the chronological test partition (15%) with forecast horizon $T+1$, Optuna Trial 11 hyperparameters, and `RANDOM_SEED = 42`.
...
Under clean conditions, the base configuration (Model D) attains marginally highest $R^2$ (0.9191), while Models A and B are tied at $R^2 = 0.9159$. The full model with DAE matches the no-DAE variant on clean data but diverges sharply under sensor corruption (Section 5.2).
...
**Denoising autoencoder robustness:** Under sensor corruption, Model A achieves $R^2 = 0.9170$ versus Model B at $R^2 = 0.8692$ ($\Delta R^2 = +0.0477$ in favour of A). Model A remains stable across conditions ($\Delta R^2 = +0.0010$ from clean to noisy), whereas Model B degrades by $\Delta R^2 = -0.0467$, indicating that auxiliary reconstruction training with $\beta = 0.05$ regularises the representation against auxiliary-channel perturbations.
...
Model B and Model D exhibit the largest $R^2$ drops under noise ($\Delta R^2 = -0.0467$ and $-0.0564$, respectively), confirming that the DAE block and full architectural stack are necessary for stable operation when auxiliary sensors are corrupted.

### 5.4 DAE Block Contribution (Model A versus Model B)

| Comparison | Clean $\Delta R^2$ (A − B) | Noisy $\Delta R^2$ (A − B) | Interpretation |
| :--- | :---: | :---: | :--- |
| DAE advantage | +0.0000 (tied on clean) | **+0.0477** (A higher on noisy) | DAE improves robustness under auxiliary-sensor corruption |
...
1. **Architectural synergy under noise.** The combination of CNN, BiLSTM, residual fusion, and self-attention elevates noisy-test performance from $R^2 = 0.8627$ (Model D) to $R^2 = 0.9170$ (Model A), a relative improvement critical for operational deployment where auxiliary sensors are imperfect.

2. **Attention-mediated temporal selection.** Disabling attention (Model C vs. A) reduces clean $R^2$ by approximately 0.009, indicating that learned weighting over historical steps outperforms uniform pooling for capturing emission spikes.

3. **Regularising effect of joint DAE training.** The DAE block matches Model B on clean data ($R^2 = 0.9159$ for both) but confers a decisive robustness advantage under corruption ($\Delta R^2 = +0.0477$ on noisy test), consistent with the denoising training protocol and moderate reconstruction weight $\beta = 0.05$.
...
This project designed, implemented, and empirically validated a five-block hybrid deep learning system for multivariate $\mathrm{PM}_{2.5}$ forecasting on the Zhang et al. (2017) Beijing hourly corpus. The architecture integrates denoising representation learning, convolutional feature extraction, bidirectional recurrent modelling, residual feature fusion, and query-independent self-attention within a compound training objective. Optuna-tuned hyperparameters yield $R^2 \approx 0.92$ on clean test data and **$R^2 \approx 0.92$ under realistic auxiliary-sensor noise** for the full model, with Model A outperforming the no-DAE variant by $\Delta R^2 = +0.0477$ where robustness is paramount.
